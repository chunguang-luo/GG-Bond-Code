...
from gg_bond_code.ipc.bridge import IPCBridge
...
from gg_bond_code.query import QueryEvent
...
class TestBridgeEventEmission:
    ...
    async def bridge_and_transport(self):
        ...
        socket_path = f"/tmp/ggbond-bridge-{uuid.uuid4().hex[:8]}.sock"
...
class TestPermissionFlow:
    ...
    async def test_permission_request_sent(self, tmp_path):
        ...
        socket_path = f"/tmp/ggbond-perm-{uuid.uuid4().hex[:8]}.sock"
        transport = IPCTransport(socket_path=socket_path)
        await transport.start()
    ...
    async def test_permission_deny(self, tmp_path):
        ...
        socket_path = f"/tmp/ggbond-deny-{uuid.uuid4().hex[:8]}.sock"
        transport = IPCTransport(socket_path=socket_path)
        await transport.start()
    ...
    async def test_permission_timeout(self, tmp_path):
        ...
        socket_path = f"/tmp/ggbond-timeout-{uuid.uuid4().hex[:8]}.sock"
        transport = IPCTransport(socket_path=socket_path)
        await transport.start()

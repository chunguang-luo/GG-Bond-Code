...
from next_code.ipc.protocol import Message, CoreToInk, InkToCore


@pytest.fixture
def socket_path():
    """Provide a short temporary socket path (macOS has 104-byte limit)."""
    return f"/tmp/nextcode-test-{uuid.uuid4().hex[:8]}.sock"
...
class TestIPCTransportWithClient:
    ...
    async def test_stale_socket_cleanup(self):
        ...
        socket_path = f"/tmp/nextcode-stale-{uuid.uuid4().hex[:8]}.sock"

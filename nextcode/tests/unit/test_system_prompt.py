...
from next_code.prompts.system import (
    build_system_prompt,
    SYSTEM_PROMPT_DYNAMIC_BOUNDARY,
)
from next_code.api.client import _split_system_prompt


@pytest.fixture
def temp_project_with_nextcode(tmp_path, monkeypatch):
    """Create a temporary project with NEXTCODE.md in .nextcode directory."""
    project_dir = tmp_path / "test_project"
    project_dir.mkdir()

    # Create .nextcode directory and NEXTCODE.md
    nextcode_dir = project_dir / ".nextcode"
    nextcode_dir.mkdir()
    nextcode_md = nextcode_dir / "NEXTCODE.md"
    nextcode_md.write_text("# Project Notes\nThis is a test project memory file.")
...
def test_static_sections_before_boundary():
    ...
    assert any("NextCode" in s for s in static_sections), "Identity section missing"
...
def test_build_system_prompt_sections_order():
    ...
    assert "NextCode" in section_texts, "Identity section missing"
...
def test_build_system_prompt_with_nextcode_md(temp_project_with_nextcode):
    """Test build_system_prompt works with NEXTCODE.md.

    Note: NEXTCODE.md is now handled by the context layer (user context),
    not directly by build_system_prompt. This test verifies that the old
    functionality (direct NEXTCODE.md inclusion) is removed.
    """
    result = build_system_prompt(temp_project_with_nextcode)
    assert isinstance(result, list)

    result_text = "\n\n".join(result)
    # NEXTCODE.md should NOT be in system prompt (moved to user context)
    assert "## NEXTCODE Project Memory" not in result_text, \
        "NEXTCODE.md should not be in system prompt (moved to user context)"


def test_build_system_prompt_without_nextcode_md():
    """Test build_system_prompt works without NEXTCODE.md.

    Note: NEXTCODE.md is now handled by user context, not system prompt.
    ...
    result_text = "\n\n".join(result)
    # Should not include NEXTCODE section if file doesn't exist
    assert "## NEXTCODE Project Memory" not in result_text, \
        "NEXTCODE.md section should not be in system prompt (moved to user context)"
...
def test_dynamic_sections_order():
    ...
    Note: Git status and NEXTCODE.md are now handled by context layers,
    ...
    # Git status and NEXTCODE.md should NOT be in dynamic sections
    ...
    assert not any("## NEXTCODE Project Memory" in s for s in dynamic_sections), \
        "NEXTCODE.md should not be in dynamic sections (moved to user context)"

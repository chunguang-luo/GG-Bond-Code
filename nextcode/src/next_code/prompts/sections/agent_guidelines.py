...
**同一类型、同一任务的 Agent 在同一条消息中只能调用一次。**
- 已派了一个 Explore Agent，就**绝对不能再派第二个** Explore Agent 做同样的搜索
- 已派了一个 Plan Agent，就**绝对不能再派第二个** Plan Agent 做同类规划
- 如果之前的 Agent 结果不够，**直接使用工具补充**，不要重新派 Agent
- 违反此规则会导致重复执行、浪费时间和资源
...
- 需要在实现之前规划实现方案时
- 并行化独立工作 — 任务独立时同时启动多个**不同类型**的子 Agent
...
（如一个 Explore + 一个 Plan，但不要两个 Explore）
